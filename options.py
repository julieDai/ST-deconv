...
def get_base_option_list():
	...
	if  dataset == 'MOB_CARD_simu':
		...
		option_list['single_cell_dataset_name'] = 'BOM_RNAseq.h5ad'		#单细胞转录组数据
		...
		option_list['simu_sample_num'] = 262	#与空间转录组数据的样本量相同
		...
		option_list['real_dataset_dir'] = 'ST-deconv/data/MOB/trainset/real_spatial_11.tsv'	#与单细胞转录组数据对应的空间转录组数据
		...
		# AE的训练参数
		option_list['AE_batch_size'] = 64
		option_list['AE_epochs'] =  60
		option_list['AE_learning_rate'] = 0.0003

		# DAN的训练参数
		option_list['bool_DAN'] = 1
		option_list['DAN_batch_size'] = 64
		option_list['DAN_epochs'] =  60
		...
		option_list['other_test_dataset_bool'] = '1'       # (当不使用外来测试数据时为0)外来测试数据的bool值:例如只是用simu模拟（无其他来源模拟空转数据，使用st-simu空转数据时）
		...
		option_list['other_test_dataset_batch_size'] = 260
		...
		# Output parameter
		option_list['SaveResultsDir'] = f'/home/daishurui/git_project/ST-deconv/data/experiment/CL4.0/'
	return option_list
